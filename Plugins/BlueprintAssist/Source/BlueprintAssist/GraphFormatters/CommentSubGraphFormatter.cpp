...
void FCommentSubGraphFormatter::FormatNode(UEdGraphNode* InitialNode)
{
	...
	{
		//UE_LOG(LogBlueprintAssist, Warning, TEXT("Root node is impure: Using format pure nodes"));
...
void FCommentSubGraphFormatter::ExpandBasedOnHeight()
{
	for (UEdGraphNode* Node : NodePool)
	{
		//UE_LOG(LogBlueprintAssist, Warning, TEXT("Expanding node %s"), *FBAUtils::GetNodeName(Node));
		...
		for (EEdGraphPinDirection Direction : { EGPD_Input, EGPD_Output })
		{
			...
			for (UEdGraphNode* Child : NodesToMove)
			{
				...
				//UE_LOG(LogBlueprintAssist, Warning, TEXT("\tExpanded node %s (%s) %d"), *FBAUtils::GetNodeName(Child), *FBAUtils::GetNodeName(Node), Direction);
	...
	for (UEdGraphNode_Comment* Comment : CommentNodes)
	{
		...
		for (UEdGraphNode* EdGraphNode : NodesUnderComment)
		{
			if (NodeTree.Contains(EdGraphNode))
			{
				...
				//UE_LOG(LogBlueprintAssist, Warning, TEXT("Comments: Added Node %s to comment %d"), *FBAUtils::GetNodeName(EdGraphNode), Comment);
			}
		}
	}
}
