...
bool FBAGraphExtender::GenerateGetter(const UEdGraph* Graph, const UEdGraphNode* Node)
{
	...
	const FName& PinCategory = SourceVariableGet->GetPinAt(0)->PinType.PinCategory;
	const FString VariableName = FBAUtils::GetVariableName(SourceVariableGet->VariableReference.GetMemberName().ToString(), PinCategory);
