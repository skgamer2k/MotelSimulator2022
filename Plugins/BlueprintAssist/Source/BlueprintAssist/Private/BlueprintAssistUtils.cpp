...
bool FBAUtils::IsExecPin(const UEdGraphPin* Pin)
{
	...
	{
		return DoesPinTypeCategoryObjectMatchName(Pin, "NiagaraParameterMap");
	}

	if (IsAnimationGraph(Graph))
	{
		return DoesPinTypeCategoryObjectMatchName(Pin, "PoseLink");
	}

	if (IsControlRigGraph(Graph))
	{
		return DoesPinTypeCategoryObjectMatchName(Pin, "ControlRigExecuteContext");
	}
...
void FBAUtils::PrintNodeInfo(UEdGraphNode* Node)
{
	...
	for (UEdGraphPin* Pin : EdGraphNode->Pins)
	{
		...
		UE_LOG(LogBlueprintAssist, Warning, TEXT("\tPin <%s> | <%s> | <%s> | <%s> | <%s> | <%s> | <%s>"),
...
bool FBAUtils::DoesPinTypeCategoryObjectMatchName(const UEdGraphPin* Pin, const FString& StructName)
{
	// Do we need to test if it is a struct?
	// Pin->PinType.PinCategory == UEdGraphSchema_K2::PC_Struct
	if (Pin->PinType.PinSubCategoryObject.IsValid())
	{
		return Pin->PinType.PinSubCategoryObject->GetName() == StructName;
	}
...
bool FBAUtils::HasMetaDataChanged(const FBAVariableDescription& OldVariable, const FBAVariableDescription& NewVariable, const FName Key)
{
...
FString FBAUtils::GetVariableName(const FString& Name, const FName& PinCategory)
{
